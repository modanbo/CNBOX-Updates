...
SWF_FILE = 'najxbox_moe_zh.swf' if storage.language_code() == 'zh' else 'najxbox_moe_en.swf'
...
class _MoeView(View):
    ...
    def populated(self):
        ...
        if self.MODE == 'lobby':
            self._apply_hangar_visibility(0)
    ...
    def _bind_lobby(self):
        ...
        try:
            g_eventBus.addListener(
                LobbySimpleEvent.HANGAR_STATUS_CHANGED,
                self._on_hangar_status,
                EVENT_BUS_SCOPE.LOBBY)
        except Exception:
            pass
        try:
            cache = datasource.items_cache()
            if cache is not None:
                cache.onSyncCompleted += self._on_items_sync
        except Exception:
            pass
    ...
    def _on_items_sync(self, *args, **kwargs):
        self._push('items-cache-synced')

    def _on_efficiency(self, *args, **kwargs):
        self._push('native-efficiency-updated')

    def _on_hangar_status(self, event):
        try:
            self._hangar_visible = bool(event.ctx.get('isVisible', False))
        except Exception:
            self._hangar_visible = False
        self._apply_hangar_visibility(0)

    def _sync_hangar_visibility(self):
        visible = False
        try:
            app = ServicesLocator.appLoader.getApp(APP_NAME_SPACE.SF_LOBBY)
            if app is not None and app.containerManager is not None:
                visible = app.containerManager.getView(
                    WindowLayer.SUB_VIEW,
                    {POP_UP_CRITERIA.VIEW_ALIAS: VIEW_ALIAS.LOBBY_HANGAR}) is not None
        except Exception:
            visible = False
        self._hangar_visible = visible
        self._apply_hangar_visibility(0)

    def _apply_hangar_visibility(self, attempt):
        if not self._alive or self.MODE != 'lobby':
            return
        try:
            if self._hangar_visible:
                self.flashObject.as_showLobbyPanel()
            else:
                self.flashObject.as_hideLobbyPanel()
            return
        except Exception:
            if attempt >= 12:
                return
        try:
            import BigWorld
            BigWorld.callback(0.25, lambda: self._apply_hangar_visibility(attempt + 1))
        except Exception:
            pass
...
def _register(alias, cls):
    if g_entitiesFactories.getSettings(alias) is None:
        g_entitiesFactories.addSettings(ViewSettings(
            alias, cls, SWF_FILE, WindowLayer.WINDOW, None, ScopeTemplates.DEFAULT_SCOPE))
